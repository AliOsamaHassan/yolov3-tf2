...
def build_example(annotation, class_map):
    ...
    if "object" in annotation:
        for obj in annotation["object"]:
            difficult = bool(int(obj["difficult"])) if obj["difficult"] != None else 0
            difficult_obj.append(int(difficult))

            truncated_value = int(obj["truncated"]) if obj["truncated"] != None else 0
            views = obj["pose"].encode("utf8") if obj["pose"] != None else None
...
def main(_argv):
    ...
    image_list = os.listdir(os.path.join(FLAGS.data_dir, "JPEGImages"))

    logging.info("Image list loaded: %d", len(image_list))
    for name in tqdm.tqdm(image_list):
        annotation_xml = os.path.join(
            FLAGS.data_dir, "Annotations", name.replace(".jpg", ".xml")
        )
